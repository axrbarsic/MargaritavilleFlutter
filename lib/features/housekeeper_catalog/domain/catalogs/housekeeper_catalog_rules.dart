...
final class HousekeeperCatalogRules {
  ...
  Housekeeper? makeHousekeeper({
    ...
    final usedIds = reservedIds ?? existing.map((value) => value.id).toSet();
  ...
  Housekeeper? renameHousekeeper({
    ...
    final trimmed = displayName.trim();
    if (normalizedId.isEmpty || trimmed.isEmpty) return null;
    final index = existing.indexWhere((value) => value.id == normalizedId);
    if (index < 0) return null;
    final canonical = _canonicalDisplayName(trimmed);
    final duplicateKey = _printedSheetDuplicateKey(canonical);
    if (existing.indexed.any(
      (entry) =>
          entry.$1 != index &&
          _printedSheetDuplicateKey(entry.$2.displayName) == duplicateKey,
    )) {
      return null;
    }
    final current = existing[index];
    if (current.displayName == canonical) return null;
    return Housekeeper(
      id: current.id,
  ...
  Housekeeper? setPalette({
    ...
    final normalizedId = housekeeperId.trim();
    final normalizedPalette = paletteKey.trim();
    if (!paletteKeys.contains(normalizedPalette)) return null;
    final index = existing.indexWhere((value) => value.id == normalizedId);
    if (index < 0) return null;
    final current = existing[index];
    if (current.paletteKey == normalizedPalette) return null;
    return Housekeeper(
      id: current.id,
      displayName: current.displayName,
      paletteKey: normalizedPalette,
      updatedAt: changedAt,
      deletedAt: current.deletedAt,
    );
  }

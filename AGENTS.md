...
- Работать только в пользовательском pull-режиме: Alex называет одну функцию,
  экран, donor behavior или проблему; реализовать ровно этот bounded vertical
  slice до проверки и установки.
- После install/result остановиться и ждать следующую команду. Не выбирать и не
  начинать следующий checkpoint автоматически, не продолжать migration queue в
  фоне и не создавать heartbeat.
- Внутри заданного slice разрешён необходимый архитектурный фундамент, но он не
  расширяет продуктовый scope. Несвязанные находки только кратко зафиксировать.
- Обычный migration checkpoint выполняет один root/writer с targeted checks и
  одним Terra correctness review перед commit. `Architecture Challenger`/Sol
  подключается только при P0/P1, риске потери данных/security, спорной
  Flutter/native границе либо после двух доказанных неудач одного причинного
  класса. Повторные `final/rereview/v2/v3` волны одного diff запрещены.
- Полный `tool/quality_gate.sh` запускать один раз перед commit/push связного
  checkpoint. После маленького reviewer-fix запускать только затронутые тесты и
  guard; новый полный gate или новый reviewer допустим лишь при новом классе
  риска.
- Нормальный лимит checkpoint: root плюс максимум один read-only sub-agent.
  Механический поиск отдавать Luna/low, обычный review — Terra; Sol/xhigh не
  использовать для рутинного переноса donor behavior.

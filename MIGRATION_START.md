...
русском языке. Работай только по конкретной команде Alex: перенеси названную
функцию/экран/поведение или исправь названную проблему как один bounded vertical
slice. После реализации, проверки, установки и короткого отчёта остановись и
жди следующую команду; не выбирай следующий checkpoint самостоятельно.
...
По умолчанию работай одним root/writer. Допускается максимум один read-only
sub-agent и только по глобальному quota-контракту.

Внутри итерации используй targeted tests. Один полный применимый gate, физическая
сборка, commit/push и запись в migration log выполняются перед завершением
связного slice. Не коммить секреты, DerivedData и чужие изменения.

Начинай только после явного задания Alex. Выполни его до проверяемого результата,
сообщи итог и остановись в ожидании следующей команды.
